...
## `integrations`
Per-user social-network linkage. **Unique** `(user_id, platform)` — one connection per platform per user. Upsert on reconnect.
| Column | Type | Notes |
|---|---|---|
...
| `access_token`, `refresh_token` | text | **AES-GCM-256 encrypted** at rest; format `${b64u(iv)}.${b64u(ct)}`. KEK = `INTEGRATION_TOKEN_KEK` |
| `token_expires_at` | int | timestamp_ms |
| `scope` | text | space-separated |
| `metadata` | json | `{ providerAccountId, raw?: {…} }` |
| `status` | text NOT NULL DEFAULT 'active' | enum: `active` / `expired` / `revoked` |
| `connected_at`, `last_sync_at` | int | |
| `created_at` / `updated_at` | int NOT NULL | |

Indexes: `(user_id)`, **UNIQUE** `(user_id, platform)` — leveraged by `onConflictDoUpdate` upsert.

**Disconnect semantics:** soft. `POST /api/integrations/disconnect` sets `status='revoked'` and nulls tokens; the row persists. Reconnect upserts back to `active`.
